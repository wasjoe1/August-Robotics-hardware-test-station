...
class TeleOpBase(object):
    ...
    def cmd_vel_str(self):
        return "x_vel: %.2f  y_vel: %.2f  rz_vel: %.2f  blocked: %s" % \
    ...
    def get_twist(self, key):
        ...
        if key == 'w' :
            ...
            print("self.trans_step is {%.3f}".format(self.trans_step))
        elif key == 'g' :
            self.trans_step = self.trans_step - 0.01
            print("self.trans_step is {%.3f}".format(self.trans_step))
        elif key == 'y' :
            self.rotat_step = self.rotat_step + 0.005
            print("self.rotat_step is {%.3f}".format(self.rotat_step))
        elif key == 'h' :
            self.rotat_step = self.rotat_step - 0.005
            print("self.rotat_step is {%.3f}".format(self.rotat_step))
        elif key == 'r' :
            ...
            print("self.acc_trans is: %.2f\tself.acc_rotate is: %.2f" %\
                (self.acc_trans, self.acc_rotate))
        elif key == 'f' :
...
if __name__=="__main__":
    ...
    print(f"{term.black_on_white}teleop_mtr.py{term.clear_eol}")
    ...
    with term.cbreak():
        while True:
            ...
            print(f"{term.move_up}{term.black_on_darkkhaki}{vel.cmd_vel_str()}"
